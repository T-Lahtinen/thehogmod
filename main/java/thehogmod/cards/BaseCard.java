...
import thehogmod.util.TriFunction;
...
import static thehogmod.util.GeneralUtils.removePrefix;
import static thehogmod.util.TextureLoader.getCardTextureString;
...
public abstract class BaseCard extends CustomCard {
    ...
    protected static String makeID(String name) { return TheHogMod.makeID(name); }
    ...
    public void onRecklessCardPlayed() {
    }

    public void onLacerationApplied() {
    }

    public void onRecklessCostChange(AbstractCard card, int costForTurn, boolean costIncreased) {
    }
    ...
    public void upgrade()
    {
        if (!upgraded)
        {
            ...
            if (this.upgradesDescription)
            {
                ...
                {
                    TheHogMod.logger.error("Card " + cardID + " upgrades description and has null upgrade description.");
                }
                else
                {
    ...
    protected Texture getPortraitImage() {
        if (this.betaArtPath != null && (Settings.PLAYTESTER_ART_MODE || UnlockTracker.betaCardPref.getBoolean(this.cardID, false))) {
            ...
            newPath = imagePath("cards/betaart/" + newPath);
